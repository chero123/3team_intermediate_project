...
class VLLMLLM(LLM):
    ...
    def __init__(
        ...
        engine_kwargs: Dict[str, Any] = dict(
            ...
            tokenizer_mode="auto",
            max_model_len=12288,    # 상황에 따라 조정 필요 (기본값은 8192: 너무 키우면 VRAM이 융단 폭격 당할 수 있음)
        )
        if quantization:
...
참고 문헌 규칙:
- 답변 마지막 문장에 참고 문헌을 포함한다.
- 형식: "참고 문헌은 A, B, C이다."처럼 한 문장으로만 작성한다.
- A, B, C는 컨텍스트 메타데이터의 filename에서 확장자를 제거한 값만 사용한다.
