...
## 메타데이터 필터링
- 질문에서 기관명/사업명/파일명 등을 추출해 메타데이터 필터로 사용한다.
- 예시:
  - `issuer = "한국생산기술연구원"`
  - `project_name = "2세대 전자조달시스템"`
- 파일명은 **확장자를 제거한 base 이름**으로 비교한다.  
  - 예: `…사업관리시스템 온라인평가.hwp` → `…사업관리시스템 온라인평가`
...
- 문맥 변경: 세션 메모리 규칙(리셋 키워드 + 명시적 메타데이터)으로 기존 문서 범위를 유지/해제
